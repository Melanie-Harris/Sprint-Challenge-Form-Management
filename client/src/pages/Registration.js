import React from "react";
import { withFormik, Formik, Form, Field } from "formik";
import * as Yup from "yup";
import axios from "axios";
import { Redirect } from "react-router-dom";
import {useLocalStateHook} from "../components/LocalStateHook";

export default function Registration(props) {
    // key ("token") is the token that is stored within localStore
    const [token, setToken] = useLocalStateHook("token"); //token string number identifying the slot that he token is stored in 
    //setToken is the way of setter token in local-store as well as the react state
    if (token) {
         return <Redirect to="/AfterRegPage" />;
    }
return(
    <Formik
        initialValues={{ username: "", password: ""}}
        onSubmit={(values, formikBag) => {
            console.log("values:", values)
            formikBag.setSubmitting(true)
            const url =
                "http://localhost:5007/api/register";
            axios
                .post(url, values)
                .then(res => {    
                    setToken("token", res.data.token)
                    formikBag.props.history.push("/AfterRegPage");
                    alert('user created successfully', res.data.username)
                    formikBag.resetForm()
                    formikBag.setSubmitting(false)
                })
                .catch(e => {
                    console.log("error:", e)
                    formikBag.setSubmitting(false)
                });}}
        validationSchema= {Yup.object().shape({
            username: Yup.string()
                .required("A username is required.  Please enter a username."),
            password: Yup.string()
                .min(6, "Your password must be a minimum length of 6.")
                .required("A password is required.  Please enter a password.")
        })}
        render={({values, errors, touched})=> (
            <Form className="form">
                <div className="form-group">
                    <label className="label">Username</label>
                    <Field
                        className="input"
                        name="username"
                        type="text"
                        autoComplete="off"
                        value={values.username}
                    />
                    {touched.username && errors.username && <p>{errors.username}</p>}
                </div>
                <div className="form-group">
                    <label className="label">Password</label>
                    <Field
                        className="input"
                        name="password"
                        type="password"
                        autoComplete="off"
                        value={values.password}
                    />
                </div>
                {touched.password && errors.password && <p>{errors.password}</p>}
                <button className="btn" type="submit">Submit &rarr;</button>
            </Form>
        )}
    >
    
    </Formik>
)
}

// export default withFormik({
//     mapPropsToValues() {
//         return {
//             username: "",
//             password: ""
//         };
//     },
    // validationSchema: Yup.object().shape({
    //     username: Yup.string()
    //         .required("A username is required.  Please enter a username."),
    //     password: Yup.string()
    //         .min(6, "Your password must be a minimum length of 6.")
    //         .required("A password is required.  Please enter a password.")
    // }),
//     handleSubmit(values, formikBag) {
//         console.log("values:", values)
//         formikBag.setSubmitting(true)
//         const url =
//             "http://localhost:5007/api/register";
//         axios
//             .post(url, values)
//             .then(res => {
//                 debugger
//                 const [token,setToken]= useLocalStateHook("token", res.data.token);
//                 setToken("token", res.data.token)
//                 formikBag.props.history.push("/AfterRegPage");
//                 alert('user created successfully', res.data.username)
//                 formikBag.resetForm()
//                 formikBag.setSubmitting(false)
               
//             })
//             .catch(e => {
//                 console.log("error:" ,e)
//                 formikBag.setSubmitting(false)
//             });
//     }
// })(Registration);
